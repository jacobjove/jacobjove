...
const SyncCalendar: NextApiHandler = async (req, res) => {
  ...
  if (await limiter.check(res, `SyncCalendar_${calendarId}`).catch(() => false)) {
    return res.status(429).json({ error: "Rate limit exceeded" });
  }

  // TODO: cut out Apollo; use Prisma directly. And don't fetch unnecessary user data.
  ...
  // Get the calendar.
  const calendar = user.calendars.find((calendar) => calendar.id === calendarId);
  ...
  if (!account) return res.status(404).json({ error: "Could not find associated account" });

  // TODO: Add support for other providers and refactor this code.
  if (calendar.provider === "google" && calendar.sourceId) {
