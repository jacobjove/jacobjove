...
const SyncCalendars: NextApiHandler = async (req, res) => {
  ...
  user.calendars.forEach(async (calendar) => {
    if (!calendar.provider || !calendar.sourceId) return;
    const account = user.accounts.find((account) => account.provider === calendar.provider) || null;
    if (!account) {
      calendarResponses[calendar.sourceId] = { error: `No ${calendar.provider} account.` };
      return;
    }
    ...
    if (calendar.provider === "google") {
      ...
      return await googleCalendar.events
        ...
        .catch((error) => {
          console.log(error);
          calendarResponses[`${calendar.sourceId}`] = { error: `${error}` };
        });
    }
  });
  return res.json(calendarResponses);
};
