...
if (ssrMode) console.warn("Accessing client-side code on server...");
...
export function initializeApolloOnClient(initialState: NormalizedCacheObject | null = null) {
  // console.info("initializeApolloOnClient with ssrMode", ssrMode);
  const _apolloClient =
    apolloClient ??
    new ApolloClient({
  ...
  // If the page has data-fetching methods that use Apollo, hydrate the initial state here.
  if (initialState) {
...
export function useApollo(pageProps: PageProps) {
  // console.log("useApollo with ssrMode", ssrMode);
  const state = pageProps[APOLLO_STATE_PROP_NAME];
  const store = useMemo(() => initializeApolloOnClient(state), [state]);
  return store;
}
