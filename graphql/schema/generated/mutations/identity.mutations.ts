...
import { CreateIdentityArgs } from "@/graphql/schema/generated/args/identity.args";
import {
  identityFragment,
  IdentityFragment,
...
export const CREATE_IDENTITY = gql`
  mutation CreateIdentity($data: IdentityCreateInput!) {
...
export const getOptimisticResponseForIdentityCreation = (data: IdentityCreateInput) => {
  const now = new Date();
  return {
    createIdentity: {
      __typename: "Identity",
...
export const UPSERT_IDENTITY = gql`
  mutation UpdateIdentity($where: IdentityWhereUniqueInput!, $data: IdentityCreateInput!) {
